...
\begin{enumerate} 
	...
	\item The string reference string ``0713272013" represents the pages that is requested. (First request page 0, then request page 7, then request...). Remember that a ``page" represents the block of virtual memory, and a ``frame" represents a block in the physical memory. If we have 8 pages, and 4 frames, we really only physically have 4 frames, and there will have to be some pages that are replaced. The 4 values inside of the brackets $[]$ will represent the physical frames that we have on RAM. 
		...
		\begin{enumerate}
			\item $[-, -, -, -]$ = $[0, -, -, -]$: Page 0 is requested. Since all 4 of the frames are empty, there is no page loaded in a frame. A page fault occurs and loads page 0 in a frame. The number of page faults that have occurred at this point is 1.
			\item $[0, -, -, -]$ = $[0, 7, -, -]$: Page 7 is requested. Page 7 is not in a frame (just like above), so a page fault occurs. The number of page faults that have occurred at this point is 2.
			\item $[0, 7, -, -]$ = $[0, 7, 1, -]$: Page 1 is requested, and a page fault occurs. The number of page faults that have occurred at this point is 3.
			\item $[0, 7, 1, -]$ = $[0, 7, 1, 3]$: Page 3 is requested, and a page fault occur. The number of page faults that have occurred at this point is 4.
		\item $[0, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 2 is requested, but there are no empty frames, nor are there any frames that currently hold page 2. A page fault occurs. Using FIFO, the oldest page will be replaced. In this case, page 0. The number of page faults that have occurred at this point is 5.
			\item $[2, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 7 is requested, but it is already loaded in a frame! Do nothing and be happy.
			\item $[2, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 2 is requested, but it is already loaded in a frame! Do nothing and be happy.
			\item $[2, 7, 1, 3]$ = $[2, 0, 1, 3]$: Page 0 is requested, but there are no empty frames, nor are there any frames that currently hold page 0. A page fault occurs. Using FIFO, the oldest page will be replaced. Page 2 was just requested, and page 7 was loaded at the very beginning. In this case, the oldest page is 7. The number of page faults that have occurred at this point is 6.
			\item $[2, 0, 1, 3]$ = $[2, 0, 1, 3]$: Page 1 is requested, but it is already loaded in a frame! Do nothing and be happy.
			\item $[2, 0, 1, 3]$ = $[2, 0, 1, 3]$: Page 3 is requested, but it is already loaded in a frame! Do nothing and be happy.
		\end{enumerate}
	...
	\item Using the same reference string in problem 5, ``0713272013", But this time with LRU, the page faults might be different. 
		...
		\begin{enumerate}
			...
			\item $[0, 7, -, -]$ = $[0, 7, 1, -]$: Page 1 is requested, and a page fault occurs. The number of page faults that have occurred at this point is 3.
			\item $[0, 7, 1, -]$ = $[0, 7, 1, 3]$: Page 3 is requested, and a page fault occur. The number of page faults that have occurred at this point is 4.
			\item $[0, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 2 is requested, but there are no empty frames, nor are there any frames that currently hold page 2. A page fault occurs. Using LRU, the least recently used page will be replaced. In this case, the time that they were last used was the time that they were also loaded. Page 0 is replaced as it was the oldest to be referenced. The number of page faults that have occurred at this point is 4.
			\item $[2, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 7 is requested, but it is already loaded in a frame! Do nothing and be happy.
			\item $[2, 7, 1, 3]$ = $[2, 7, 1, 3]$: Page 2 is requested, but it is already loaded in a frame! Do nothing and be happy.
			\item $[2, 7, 1, 3]$ = $[2, 7, 0, 3]$: Page 0 is requested, but there are no empty frames, nor are there any frames that currently hold page 0. A page fault occurs. Using LUR, the least recently used page will be replaced. Page 2 was just requested, and page 7 was also requested just before that! Pages 1 and 3 are the least recently used ones, but since page 1 was added first, it is technically older than page 3, so it is chosen. The number of page faults that have occurred at this point is 6.
			\item $[2, 7, 0, 3]$ = $[2, 7, 0, 1]$: Page 1 is requested, but there are no empty frames, nor are there any frames that currently hold page 1. A page fault occurs. Using LUR, the least recently used page will be replaced. Page 0 was just updated, before that was page 2, and before that was page 7. This leaves us with page 3 as being the oldest one requested, therefore, it is chosen to be replaced. The number of page faults that have occurred at this point is 7.
			\item $[2, 7, 0, 1]$ = $[2, 3, 0, 1]$: Page 3 is requested, and a page fault occurs. I can't help you if you don't know what goes on here, and I am not going to spell it out again. The oldest page here is 7. It is chosen for a page fault. The number of page faults that have occurred at this point is 8.
		\end{enumerate}
	...
	\item If the first part of each UNIX file was kept in the same disk block as it's i-node, this would reduce the  time it would take to seek for small files. If the file is small, then a seek would take a lot of time in comparison to the rest of the time it would take to read the file. If the file being read was large, that seek time would be less of an impact on the file in comparison to the time it would take to read the rest of that large file.If the first data block was right next to the inode, then the seek wouldn't occur initially, which may potentially save some time.
	...
	\item
		For every block read, we must account for a seek, then the rotational latency, and then the time to transfer.I am not sure why the cylinders, tracks and blocks are given. They don't seem to change the problem if we are given the rest of the seek times.
		\begin{enumerate}
			\item: No effort to cluster blocks: (Average seek of 5 milliseconds)
				The time per block would be 5 ms for the seek, 10 ms for the rotational latency, and 0.02 ms for the actual data transfer. In total that is 15.02 ms needed per one block read. For 100 blocks, that will result in 1,502 ms to read the whole file.
			\item: Some effort to cluster blocks: (Average seek of 100 microseconds, aka 0.1 milliseconds)
				Only the first read will be slow. This reflects the time to read a block, which would take 15.02 ms. The remaining 99 blocks will be more efficient. In those block reads, the time to seek would change to 0.1 ms, resulting in a time of 10.12 ms needed per block. For 99 blocks, that will result in 1,011.88 ms. Adding that to the initial seek of 15.02 ms, you get a grand total of 1,016.9 ms to read the entire file.
		\end{enumerate}

	\item Carolyn's answer does not make that much sense; Elanor is correct. Even if it is faster to fetch a new copy of the inode, you would still have to go back and clean the old inode up to prevent things from trying to point to the old inode. If there is a directory with a bunch of directory entries that point to the old inode, then you would have to go an update those numbers to be the new inode. How do you know what points to a particular inode? For all you know, all the files could be links to said inode number. It is best to not make a duplicate inode with a new inode number, and just seek to the original inode.
\end{enumerate} 
\end{document}
